...
/**
 * Circuit Breaker Configuration Prevents cascading failures by breaking the circuit when services
 * are down Implements trust by design - failures are handled gracefully
 */
...
public class CircuitBreakerConfig {
    ...
    public CircuitBreakerRegistry circuitBreakerRegistry() {
        final io.github.resilience4j.circuitbreaker.CircuitBreakerConfig config =
                io.github.resilience4j.circuitbreaker.CircuitBreakerConfig.custom()
                        .failureRateThreshold(50) // Open circuit if 50% of requests fail
                        .waitDurationInOpenState(
                                Duration.ofSeconds(30)) // Wait 30s before half-open
        ...
        final io.github.resilience4j.circuitbreaker.CircuitBreakerConfig config =
                io.github.resilience4j.circuitbreaker.CircuitBreakerConfig.custom()
                        .failureRateThreshold(
                                70) // Higher threshold for external service (70% failure rate)
                        .waitDurationInOpenState(
                                Duration.ofMinutes(2)) // Wait 2 minutes before half-open
                        .slidingWindowSize(30) // Look at last 30 calls (more buffer)
                        .minimumNumberOfCalls(
                                15) // Need at least 15 calls before opening (prevents premature
                        // opening)
