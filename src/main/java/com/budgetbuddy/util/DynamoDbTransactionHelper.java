...
/**
 * DynamoDB Transaction Helper Provides support for multi-item atomic operations using DynamoDB
 * Transactions
 *
 * <p>CRITICAL: DynamoDB Transactions ensure atomicity across multiple items - All operations
 * succeed or all fail (ACID properties) - Eliminates TOCTOU windows and race conditions - Maximum
 * 25 items per transaction
 */
public final class DynamoDbTransactionHelper {
    ...
    public static TransactWriteItemsResponse executeTransaction(
        ...
        if (items.size() > MAX_TRANSACTION_ITEMS) {
            throw new IllegalArgumentException(
                    String.format(
                            "DynamoDB transactions support maximum %d items, got %d",
    ...
    public static TransactWriteItem createPutItem(
        ...
        final Put.Builder putBuilder = Put.builder().tableName(tableName).item(item);

        if (conditionExpression != null && !conditionExpression.isEmpty()) {
            putBuilder.conditionExpression(conditionExpression);
        }

        return TransactWriteItem.builder().put(putBuilder.build()).build();
    }
    ...
    public static TransactWriteItem createUpdateItem(
        ...
        final Update.Builder updateBuilder =
                Update.builder().tableName(tableName).key(key).updateExpression(updateExpression);
        ...
        return TransactWriteItem.builder().update(updateBuilder.build()).build();
    }

    /**
     * Example: Create transaction and update budget in a single atomic operation This ensures both
