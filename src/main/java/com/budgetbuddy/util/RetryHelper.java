...
/**
 * Retry Helper Utility Provides retry logic with exponential backoff for batch operations and
 * DynamoDB operations
 *
...
public final class RetryHelper {
    ...
    public static <T> T executeWithRetry(
        ...
        while (attempt <= maxRetries) {
            try {
                ...
                currentDelay =
                        Duration.ofMillis((long) (currentDelay.toMillis() * backoffMultiplier));
                attempt++;
            }
        }
    ...
    public static <T> T executeWithRetry(final Supplier<T> operation) {
        return executeWithRetry(
                operation, DEFAULT_MAX_RETRIES, DEFAULT_INITIAL_DELAY, DEFAULT_BACKOFF_MULTIPLIER);
    }
    ...
    public static <T> T executeWithRetry(final Supplier<T> operation, final int maxRetries) {
        return executeWithRetry(
                operation, maxRetries, DEFAULT_INITIAL_DELAY, DEFAULT_BACKOFF_MULTIPLIER);
    }
    ...
    /**
     * Execute DynamoDB operation with retry for throttling and service errors Uses longer delays
     * for DynamoDB operations (throttling can be persistent)
     */
    ...
    public static <T> T executeDynamoDbWithRetry(
            final Supplier<T> operation, final int maxRetries) {
        return executeWithRetry(
                operation, maxRetries, Duration.ofMillis(200), DEFAULT_BACKOFF_MULTIPLIER);
    }
}
