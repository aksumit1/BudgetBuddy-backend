...
public final class StringUtils {
     ...
     */
    public static String normalizeMerchantName(final String merchantName) {
        if (merchantName == null || merchantName.isBlank()) {
            return merchantName;
        }

        String normalized = merchantName.trim().toUpperCase(Locale.ROOT);
        ...
        // CRITICAL FIX: Handle # with or without space before it (e.g., "SAFEWAY#1444" or "SAFEWAY
        // #1444")
        normalized =
                normalized.replaceAll(
                        "#\\s*\\d+", ""); // Remove #123, # 123, etc. (with or without space)
        ...
        normalized =
                normalized.replaceAll(
                        "\\s+\\d{4,}(?=\\s|$)",
                        ""); // Remove long numeric IDs (4+ digits) at end or followed by space
        ...
        normalized =
                normalized.replaceAll("\\s*\\*\\s*", " "); // Also handle * with spaces around it
