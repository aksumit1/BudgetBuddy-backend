...
public class HIPAAComplianceService {
    ...
    public void logPHIAccess(
        ...
        if (!authorized) {
            LOGGER.warn(
                    "HIPAA: Unauthorized PHI access attempt: User={}, Type={}, Action={}",
                    userId,
                    phiType,
                    action);
    ...
    /** §164.312(b) - Audit Controls Log all PHI access and modifications */
    ...
    /** §164.312(c)(1) - Integrity Ensure PHI is not improperly altered or destroyed */
    public void logPHIModification(
        ...
        auditLogService.logPHIModification(
                userId, phiId, modificationType, beforeValue, afterValue);
        putMetric("PHIModification", 1.0, Map.of("ModificationType", modificationType));
    }

    /** §164.312(e)(1) - Transmission Security Encrypt PHI during transmission */
    public void logPHITransmission(
            final String userId, final String destination, final boolean encrypted) {
        if (!encrypted) {
            LOGGER.error(
                    "HIPAA VIOLATION: PHI transmitted without encryption: User={}, Destination={}",
                    userId,
                    destination);
    ...
    /** §164.312(a)(2)(iv) - Automatic Logoff Implement automatic logoff after inactivity */
    public void checkSessionTimeout(final String userId, final long lastActivityTime) {
        final long inactivityMinutes = (Instant.now().getEpochSecond() - lastActivityTime) / 60;
        if (inactivityMinutes > 15) { // 15 minute timeout
            LOGGER.warn(
                    "HIPAA: Session timeout exceeded: User={}, Inactivity={} minutes",
                    userId,
                    inactivityMinutes);
    ...
    /** §164.400-414 - Breach Notification Detect and report PHI breaches */
    public void reportBreach(
            final String userId,
            final String phiId,
            final String breachType,
            final String details) {
        final BreachReport report = new BreachReport();
        ...
        LOGGER.error(
                "HIPAA BREACH DETECTED: User={}, PHI={}, Type={}, Details={}",
                userId,
                phiId,
                breachType,
                details);
    ...
    /**
     * Trigger automated breach notification workflow §164.400-414 - Breach Notification
     * Requirements
     */
    ...
    /** §164.308(a)(3) - Workforce Security Ensure workforce members have appropriate access */
    ...
    /**
     * §164.308(a)(4) - Information Access Management Implement policies for access to PHI Uses
     * role-based access control to determine if user can access specific PHI type
     */
    public boolean checkPHIAccessPolicy(final String userId, final String phiType) {
        ...
        // Get user roles
        final java.util.Set<String> roles = identityVerificationService.getUserRoles(userId);
    ...
    /** §164.312(d) - Person or Entity Authentication Implement procedures to verify identity */
    ...
    public static class BreachReport {
        ...
        public void setPhiId(final String phiId) {
            this.phiId = phiId;
        }
        ...
        public void setBreachType(final String breachType) {
