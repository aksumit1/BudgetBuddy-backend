...
/**
 * DynamoDB Repository for Import History
 * Tracks all import operations for audit trail and history
 */
@Repository
@org.springframework.context.annotation.DependsOn({"dynamoDBTableManager", "dynamoDbEnhancedClient"})
public class ImportHistoryRepository {
    ...
    public ImportHistoryRepository(
        ...
        try {
            this.importHistoryTable = enhancedClient.table(this.tableName, TableSchema.fromBean(ImportHistoryTable.class));
            this.userIdIndex = importHistoryTable.index("UserIdIndex");
        } catch (Exception e) {
            throw new IllegalStateException(
                    String.format("Failed to initialize ImportHistoryRepository for table '%s'. " +
    ...
    public Optional<ImportHistoryTable> findById(final String importId) {
        ...
        String normalizedId = com.budgetbuddy.util.IdGenerator.normalizeUUID(importId);
        ImportHistoryTable importHistory = importHistoryTable.getItem(
                Key.builder().partitionValue(normalizedId).build());
        return Optional.ofNullable(importHistory);
    }

    @Cacheable(value = "importHistory", key = "#userId", unless = "#result == null || #result.isEmpty()")
    public List<ImportHistoryTable> findByUserId(final String userId) {
        if (userId == null || userId.isEmpty()) {
            return List.of();
        }
        List<ImportHistoryTable> results = new ArrayList<>();
        try {
            SdkIterable<software.amazon.awssdk.enhanced.dynamodb.model.Page<ImportHistoryTable>> pages =
                    userIdIndex.query(QueryConditional.keyEqualTo(Key.builder().partitionValue(userId).build()));
            for (software.amazon.awssdk.enhanced.dynamodb.model.Page<ImportHistoryTable> page : pages) {
                for (ImportHistoryTable item : page.items()) {
                    if (item != null) {
                        results.add(item);
                    }
                }
    ...
    public void delete(final String importId) {
        ...
        String normalizedId = com.budgetbuddy.util.IdGenerator.normalizeUUID(importId);
        com.budgetbuddy.util.RetryHelper.executeDynamoDbWithRetry(() -> {
            importHistoryTable.deleteItem(Key.builder().partitionValue(normalizedId).build());
            return null;
        });
    }
}