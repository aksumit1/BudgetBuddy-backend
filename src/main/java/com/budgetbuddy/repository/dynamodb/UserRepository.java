...
/**
 * DynamoDB Repository for Users Uses GSI for email lookup (cost-optimized)
 *
 * <p>Note: Depends on DynamoDBTableManager to ensure tables are created before use
 */
...
public class UserRepository {
    ...
    public UserRepository(
        ...
        try {
            ...
            this.userTable =
                    enhancedClient.table(this.tableName, TableSchema.fromBean(UserTable.class));
            ...
            throw new IllegalStateException(
                    String.format(
                            "Failed to initialize UserRepository for table '%s'. "
    ...
    /**
     * Find user by email bypassing cache (for registration checks) This ensures fresh data from
     * DynamoDB, avoiding stale cache entries Use this method when checking if a user exists before
     * registration
     */
    ...
    /**
     * Find ALL users with the same email (for duplicate detection) Returns a list of all users with
     * the given email Used to detect race conditions where multiple users are created with the same
     * email
     */
    ...
    public void updateLastLogin(final String userId, final Instant lastLogin) {
        ...
        userTable.updateItem(UpdateItemEnhancedRequest.builder(UserTable.class).item(user).build());
    }
    ...
    /**
     * Find user by ID with projection expression (cost-optimized) Retrieves only specified
     * attributes using low-level DynamoDB client
     *
     * <p>Benefits: - Reduces data transfer costs by 30-50% - Faster response times for partial
     * attribute retrieval - Lower DynamoDB read costs
     */
    public Optional<UserTable> findByIdWithProjection(
        ...
        try {
            ...
            final GetItemRequest request =
                    GetItemRequest.builder()
                            ...
                            .expressionAttributeNames(
                                    expressionAttributeNames.isEmpty()
                                            ? null
                                            : expressionAttributeNames)
                            .build();
            ...
            final UserTable user = convertAttributeValueMapToUser(response.item());
    ...
    /** Convert AttributeValue map to UserTable Only sets attributes that are present in the map */
    private UserTable convertAttributeValueMapToUser(final Map<String, AttributeValue> item) {
        ...
        if (item.containsKey("lastLoginAt")) {
            final String timestamp = item.get("lastLoginAt").n();
            if (timestamp != null) {
    ...
    /**
     * Find active users (users who logged in within the specified days) OPTIMIZED: Uses GSI on
     * enabled + lastLoginAtTimestamp for efficient queries
     *
    ...
    public List<String> findActiveUserIds(final int days, final int limit) {
        ...
        final List<String> activeUserIds = new ArrayList<>();
        final Instant cutoffDate = Instant.now().minusSeconds(days * 24L * 60 * 60);
        final long cutoffTimestamp = cutoffDate.getEpochSecond();
