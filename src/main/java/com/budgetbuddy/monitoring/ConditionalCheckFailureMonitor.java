...
/**
 * Monitoring service for ConditionalCheckFailedException Tracks occurrences to identify race
 * conditions and conflicts
 *
 * <p>MEDIUM PRIORITY: Provides visibility into concurrent update conflicts
 */
@Component
public class ConditionalCheckFailureMonitor {

    private static final Logger LOGGER =
            LoggerFactory.getLogger(ConditionalCheckFailureMonitor.class);
    ...
    public void recordFailure(
        ...
        final String operationKey = operation != null ? operation : "unknown";
        final String tableKey = tableName != null ? tableName : "unknown";
        ...
        if (count % 10 == 0) {
            LOGGER.warn(
                    "High frequency of conditional check failures detected - Operation: {}, Count: {}",
                    operationKey,
                    count);
        }
    }
    ...
    public long getFailureCount(final String operation) {
        final AtomicLong count = failureCounts.get(operation);
        return count != null ? count.get() : 0;
    }

    /** Get failure count for a table */
    public long getFailureCountByTable(final String tableName) {
        final AtomicLong count = failureCountsByTable.get(tableName);
        return count != null ? count.get() : 0;
    }
