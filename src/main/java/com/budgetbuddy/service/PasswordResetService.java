...
/** Service for password reset with email verification Uses 6-digit codes sent via email */
@Service
public class PasswordResetService {
    ...
    public PasswordResetService(
            final UserRepository userRepository, final EmailNotificationService emailService) {
    ...
    public void requestPasswordReset(final String email) {
        ...
        final Instant expiresAt = Instant.now().plusSeconds(CODE_EXPIRY_MINUTES * 60);

        // Store code info
        resetCodes.put(email.toLowerCase(Locale.ROOT), new ResetCodeInfo(code, expiresAt, 0));

        // Send email with code
        final String subject = "BudgetBuddy Password Reset Code";
        final String body =
                String.format(
                        "<html><body>"
        ...
        try {
            ...
            if (!emailSent) {
                LOGGER.error(
                        "Failed to send password reset email to: {}. Email service returned false.",
                        email);
    ...
    public void verifyResetCode(final String email, final String code) {
        ...
        final ResetCodeInfo codeInfo = resetCodes.get(emailKey);

        if (codeInfo == null) {
            throw new AppException(
                    ErrorCode.INVALID_INPUT,
                    "No reset code found for this email. Please request a new code.");
        }
        ...
        if (!codeInfo.getCode().equals(code)) {
            ...
            throw new AppException(
                    ...
                    "Invalid verification code. Attempts remaining: "
                            + (MAX_ATTEMPTS - codeInfo.getAttempts()));
        }
    ...
    /**
     * Reset password with verified code BREAKING CHANGE: Client salt removed - Zero Trust
     * architecture
     */
    public void resetPassword(final String email, final String code, final String passwordHash) {
        ...
        if (codeInfo == null || !codeInfo.isVerified() || !codeInfo.getCode().equals(code)) {
            throw new AppException(
                    ErrorCode.INVALID_INPUT,
                    "Invalid or unverified code. Please start the reset process again.");
        }
    ...
    private String generateCode() {
        final int code = 100_000 + random.nextInt(900_000); // 100000 to 999999
