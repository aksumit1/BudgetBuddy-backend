...
/**
 * Unit Tests for AuthService Password Format Validation Tests that only secure password_hash + salt
 * format is accepted, not plaintext passwords
 */
...
class AuthServicePasswordFormatTest {
    ...
    @Mock private com.budgetbuddy.security.JwtTokenProvider jwtTokenProvider;

    @Mock private CacheWarmingService cacheWarmingService;
    ...
    @Test
    void testAuthenticateWithSecureFormatShouldSucceed() {
        ...
        when(passwordHashingService.verifyClientPassword(
                        testPasswordHash, testUser.getPasswordHash(), testUser.getServerSalt()))
                .thenReturn(true);
    ...
    @Test
    void testAuthenticateWithPlaintextPasswordShouldThrowException() {
        ...
        assertTrue(
                ex.getErrorCode() == ErrorCode.INVALID_INPUT
                        || ex.getMessage().contains("password_hash")
                        || ex.getMessage().contains("secure format"));
    }

    @Test
    void testAuthenticateWithMissingPasswordHashShouldThrowException() {
        // Given - Request with no password_hash (BREAKING CHANGE: Client salt removed, only
