...
/**
 * Comprehensive tests for enhanced username detection features:
 * 1. Excluded word filtering (reject if any word is excluded)
 * 2. Special character and domain checks (/, .COM, etc.)
 * 3. Transaction line detection (skip transaction lines)
 * 4. All-caps only selection (deprioritize mixed-case)
 * 5. Country name exclusions (USA, UK, INDIA, etc.)
 * 6. Two-letter code detection (DL, INT, etc.)
 * 7. Single-letter ending detection (reject patterns like "DELHI DL K")
 * 
 * Also tests error handling, edge cases, boundary conditions, and thread safety
 */
...
public class PDFImportServiceUsernameDetectionEnhancedTest {
    ...
    void setUp() throws Exception {
        ...
        isTransactionLine = PDFImportService.class.getDeclaredMethod("isTransactionLine", String.class);
    ...
    void testExcludedWordFiltering_AnywhereInLine() throws Exception {
        // These should be rejected because they contain excluded words
        assertFalse((Boolean) isValidNameFormat.invoke(pdfImportService, "Standard Purchases"),
            "Should reject 'Standard Purchases' (contains 'purchases')");
        assertFalse((Boolean) isValidNameFormat.invoke(pdfImportService, "Credits Amount"),
            "Should reject 'Credits Amount' (contains 'credits' and 'amount')");
        assertFalse((Boolean) isValidNameFormat.invoke(pdfImportService, "Payment History"),
            "Should reject 'Payment History' (contains 'payment' and 'history')");
        assertFalse((Boolean) isValidNameFormat.invoke(pdfImportService, "Transaction Details"),
    ...
    void testExcludedWordFiltering_FinancialTerms(String name) throws Exception {
        assertFalse((Boolean) isValidNameFormat.invoke(pdfImportService, name),
            "Should reject: " + name);
    }

    @Test
    @DisplayName("Should allow valid names that don't contain excluded words")
    void testExcludedWordFiltering_ValidNames() throws Exception {
    ...
    void testSpecialCharacter_ForwardSlash() throws Exception {
        assertFalse((Boolean) isValidNameFormat.invoke(pdfImportService, "HULU.COM/BILL"),
    ...
    void testSpecialCharacter_Backslash() throws Exception {
        assertFalse((Boolean) isValidNameFormat.invoke(pdfImportService, "John\\Doe"),
            "Should reject 'John\\Doe' (contains backslash)");
        assertFalse((Boolean) isValidNameFormat.invoke(pdfImportService, "Path\\To\\Name"),
    ...
    void testDomainPatterns() throws Exception {
        assertFalse((Boolean) isValidNameFormat.invoke(pdfImportService, "HULU.COM"),
            "Should reject 'HULU.COM' (contains .COM)");
        assertFalse((Boolean) isValidNameFormat.invoke(pdfImportService, "example.NET"),
            "Should reject 'example.NET' (contains .NET)");
        assertFalse((Boolean) isValidNameFormat.invoke(pdfImportService, "test.ORG"),
            "Should reject 'test.ORG' (contains .ORG)");
        assertFalse((Boolean) isValidNameFormat.invoke(pdfImportService, "site.BILL"),
            "Should reject 'site.BILL' (contains .BILL)");
    }

    @Test
    @DisplayName("Should allow valid names without special characters or domains")
    void testSpecialCharacter_ValidNames() throws Exception {
        assertTrue((Boolean) isValidNameFormat.invoke(pdfImportService, "JOHN DOE"),
            "Should allow 'JOHN DOE'");
        assertTrue((Boolean) isValidNameFormat.invoke(pdfImportService, "Mary-Jane Smith"),
    ...
    void testIsTransactionLine_WithDates() throws Exception {
        assertTrue((Boolean) isTransactionLine.invoke(pdfImportService, "01/15/2024 LULULEMON ATHLETICA 123.45"),
            "Should detect transaction line with date");
        assertTrue((Boolean) isTransactionLine.invoke(pdfImportService, "12-25-2023 AMAZON 99.99"),
            "Should detect transaction line with date (dash format)");
    }

    @Test
    @DisplayName("Should detect transaction lines with amounts")
    void testIsTransactionLine_WithAmounts() throws Exception {
        assertTrue((Boolean) isTransactionLine.invoke(pdfImportService, "LULULEMON ATHLETICA $123.45"),
            "Should detect transaction line with currency symbol");
        assertTrue((Boolean) isTransactionLine.invoke(pdfImportService, "AMAZON 99.99"),
            "Should detect transaction line with decimal amount");
        assertTrue((Boolean) isTransactionLine.invoke(pdfImportService, "STARBUCKS 1,234.56"),
    ...
    void testIsTransactionLine_WithKeywords() throws Exception {
        assertTrue((Boolean) isTransactionLine.invoke(pdfImportService, "PURCHASE 123.45"),
            "Should detect transaction line with 'PURCHASE' keyword");
        assertTrue((Boolean) isTransactionLine.invoke(pdfImportService, "PAYMENT 99.99"),
            "Should detect transaction line with 'PAYMENT' keyword");
    }

    @Test
    @DisplayName("Should not detect regular names as transaction lines")
    void testIsTransactionLine_RegularNames() throws Exception {
        assertFalse((Boolean) isTransactionLine.invoke(pdfImportService, "JOHN DOE"),
            "Should not detect 'JOHN DOE' as transaction line");
        assertFalse((Boolean) isTransactionLine.invoke(pdfImportService, "Mary Jane Smith"),
    ...
    void testFindUsernameCandidates_SkipsTransactionLines() throws Exception {
        String[] lines = {
            "JOHN DOE",
        ...
        assertFalse(candidates.contains("LULULEMON ATHLETICA"), 
            "Should not include transaction line merchant name");
        assertFalse(candidates.contains("DELTA AIR LINES"), 
            "Should not include transaction line merchant name");
    }
    ...
    void testAllCapsOnly_WithMultipleCandidates() throws Exception {
        String[] lines = {
            "JOHN DOE",  // All-caps
            "John Doe",  // Title case
    ...
    void testAllCapsOnly_NoAllCapsCandidates() throws Exception {
        String[] lines = {
    ...
    void testAllCapsOnly_WithAccountHolderName() throws Exception {
        String[] lines = {
            "JOHN DOE",  // All-caps
    ...
    void testCountryNameExclusions(String countryName) throws Exception {
        assertFalse((Boolean) isValidNameFormat.invoke(pdfImportService, countryName),
            "Should reject country name: " + countryName);
    }

    @Test
    @DisplayName("Should reject names containing country names")
    void testCountryNameExclusions_WithinName() throws Exception {
        assertFalse((Boolean) isValidNameFormat.invoke(pdfImportService, "JOHN USA"),
            "Should reject 'JOHN USA' (contains country code)");
        assertFalse((Boolean) isValidNameFormat.invoke(pdfImportService, "MARY UK"),
            "Should reject 'MARY UK' (contains country code)");
        assertFalse((Boolean) isValidNameFormat.invoke(pdfImportService, "JANE INDIA"),
    ...
    void testTwoLetterCodes_AllCaps(String code) throws Exception {
        // In all-caps context, these should be rejected
        assertFalse((Boolean) isValidNameFormat.invoke(pdfImportService, "DELHI " + code + " K"),
            "Should reject 'DELHI " + code + " K' (contains 2-letter code in all-caps)");
        assertFalse((Boolean) isValidNameFormat.invoke(pdfImportService, "SEATTLE " + code),
    ...
    void testTwoLetterCodes_FalsePositives() throws Exception {
        assertFalse((Boolean) isValidNameFormat.invoke(pdfImportService, "DELHI DL K"),
            "Should reject 'DELHI DL K' (contains DL code)");
        assertFalse((Boolean) isValidNameFormat.invoke(pdfImportService, "SEATTLE-TACOMA INT DL H"),
    ...
    void testTwoLetterCodes_ValidNames() throws Exception {
        assertTrue((Boolean) isValidNameFormat.invoke(pdfImportService, "JOHN DOE"),
            "Should allow 'JOHN DOE'");
        assertTrue((Boolean) isValidNameFormat.invoke(pdfImportService, "MARY JANE SMITH"),
            "Should allow 'MARY JANE SMITH'");
    ...
    void testSingleLetterEnding_Reject() throws Exception {
        assertFalse((Boolean) isValidNameFormat.invoke(pdfImportService, "DELHI DL K"),
            "Should reject 'DELHI DL K' (ends with single letter 'K')");
        assertFalse((Boolean) isValidNameFormat.invoke(pdfImportService, "SEATTLE-TACOMA INT DL H"),
            "Should reject 'SEATTLE-TACOMA INT DL H' (ends with single letter 'H')");
        assertFalse((Boolean) isValidNameFormat.invoke(pdfImportService, "LOCATION CODE B"),
            "Should reject 'LOCATION CODE B' (ends with single letter 'B')");
    }

    @Test
    @DisplayName("Should allow single letters in valid name patterns")
    void testSingleLetterEnding_ValidPatterns() throws Exception {
        assertTrue((Boolean) isValidNameFormat.invoke(pdfImportService, "John J. Smith"),
            "Should allow 'John J. Smith' (middle initial with period)");
        assertTrue((Boolean) isValidNameFormat.invoke(pdfImportService, "Mary K. Doe"),
    ...
    void testSingleLetterEnding_SingleWord() throws Exception {
    ...
    void testEdgeCases_NullInput() throws Exception {
    ...
    void testEdgeCases_EmptyString() throws Exception {
    ...
    void testEdgeCases_VeryLongString() throws Exception {
    ...
    void testEdgeCases_UnicodeCharacters() throws Exception {
    ...
    void testBoundaryConditions_OneWord() throws Exception {
    ...
    void testBoundaryConditions_FiveWords() throws Exception {
        assertTrue((Boolean) isValidNameFormat.invoke(pdfImportService, "John Mary Jane Smith Doe"),
    ...
    void testBoundaryConditions_MoreThanFiveWords() throws Exception {
        assertFalse((Boolean) isValidNameFormat.invoke(pdfImportService, "John Mary Jane Smith Doe Jr"),
    ...
    void testErrorHandling_InvalidIndices() throws Exception {
        String[] lines = {"JOHN DOE"};
    ...
    void testErrorHandling_NullArray() throws Exception {
        assertDoesNotThrow(() -> {
            @SuppressWarnings("unchecked")
            List<String> result = (List<String>) findUsernameCandidates.invoke(
                pdfImportService, null, 0, 1, 3);
    ...
    void testErrorHandling_NullAccountHolderName() throws Exception {
        String[] lines = {"JOHN DOE", "Card Member: JANE SMITH"};
    ...
    void testRealWorldFalsePositives() throws Exception {
        ...
        assertFalse((Boolean) isValidNameFormat.invoke(pdfImportService, "LULULEMON ATHLETICA USA B"),
            "Should reject 'LULULEMON ATHLETICA USA B'");
        assertFalse((Boolean) isValidNameFormat.invoke(pdfImportService, "Credits Amount"),
            "Should reject 'Credits Amount'");
        assertFalse((Boolean) isValidNameFormat.invoke(pdfImportService, "DELHI DL K"),
            "Should reject 'DELHI DL K'");
        assertFalse((Boolean) isValidNameFormat.invoke(pdfImportService, "DELTA AIR LINES"),
    ...
    void testRealWorldFalsePositives_TransactionLines() throws Exception {
        // These should be detected as transaction lines
        assertTrue((Boolean) isTransactionLine.invoke(pdfImportService, "DELTA AIR LINES $99.99"),
    ...
    void testThreadSafety_ConcurrentDetection() throws Exception {
        int numThreads = 10;
        ...
        assertEquals(0, errorCount.get(), 
            "Should not have any errors in concurrent execution");
    ...
    void testThreadSafety_ConcurrentValidation() throws Exception {
        int numThreads = 20;
        ...
        assertEquals(0, errorCount.get(), 
            "Should not have any errors in concurrent validation");
    }
    ...
    void testIntegration_RealisticScenario() throws Exception {
        ...
        // Note: If this fails, "JANE SMITH" might not be in the candidate list or might be filtered out
        assertNotNull(username2, "Should detect username for second section. " +
    ...
    void testIntegration_MultiUserStatement() throws Exception {
        String[] lines = {
            "JOHN DOE",  // Index 0
            "123 Main St",  // Index 1
