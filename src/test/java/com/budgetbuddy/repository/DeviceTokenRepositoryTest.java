 ...
 * Tests edge cases, race conditions, and concurrent operations
 * 
 * NOTE: This is an integration test that requires Docker/Testcontainers
 * For unit testing, use mocked repositories in service tests
 * Run integration tests separately: mvn verify -Pintegration-test
 * 
 * CRITICAL: Testcontainers requires Docker. If Docker is not available,
 * the test will fail during class initialization. This test is disabled
 * in CI environments where Docker may not be available.
 */
...
class DeviceTokenRepositoryTest {
    ...
    static org.testcontainers.containers.localstack.LocalStackContainer localStack = 
            new org.testcontainers.containers.localstack.LocalStackContainer(
                    org.testcontainers.utility.DockerImageName.parse("localstack/localstack:latest"))
            .withServices(org.testcontainers.containers.localstack.LocalStackContainer.Service.DYNAMODB);
    ...
    void setUp() {
        ...
        if (localStack == null || !localStack.isRunning()) {
            org.junit.jupiter.api.Assumptions.assumeTrue(false, "LocalStack container not available - skipping test");
            return;
        }
        
        dynamoDbClient = DynamoDbClient.builder()
                .endpointOverride(localStack.getEndpointOverride(org.testcontainers.containers.localstack.LocalStackContainer.Service.DYNAMODB))
    ...
    void testSaveAndRetrieve() {
        ...
        DeviceTokenTable token = new DeviceTokenTable();
        ...
        Optional<DeviceTokenTable> retrieved = repository.findByUserIdAndDeviceToken(userId, deviceToken);
    ...
    void testFindEnabledTokens() {
        ...
        List<DeviceTokenTable> enabledTokens = repository.findEnabledByUserId(userId);
    ...
    void testEmptyUserId() {
        List<DeviceTokenTable> tokens = repository.findByUserId("");
    ...
    void testUpdateLastUsed() throws InterruptedException {
        ...
        Optional<DeviceTokenTable> updated = repository.findByUserIdAndDeviceToken(userId, deviceToken);
    ...
    void testDisableToken() {
        ...
        Optional<DeviceTokenTable> disabled = repository.findByUserIdAndDeviceToken(userId, deviceToken);
    ...
    void testMultipleDevicesPerUser() {
        ...
        List<DeviceTokenTable> allTokens = repository.findByUserId(userId);
